...
i32 alphabeta(Position &pos,
    ...
    for (i32 i = 0; i < num_moves; ++i) {
        ...
        if (in_qsearch)
        {
            if (!in_check && static_eval + 50 + gain < alpha) {
                best_score = alpha;
                break;
            }

            if (static_eval + gain > beta + 128)
            {
                best_score = static_eval + gain;
                break;
            }
        }
        // Forward futility pruning
        else if (depth < 8 && !in_check && !(move == tt_move) && static_eval + 100 * depth + gain < alpha) {
            best_score = alpha;
            break;
        }
