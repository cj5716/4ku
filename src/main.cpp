...
const i32 max_material[] = {129, 413, 442, 757, 1463, 0, 0};
const i32 material[] = {S(92, 129), S(315, 413), S(331, 442), S(418, 757), S(799, 1463), 0};
const i32 pst_rank[][8] = {
    {0, S(-3, 0), S(-3, -1), S(-1, -1), S(2, 0), S(5, 2), 0, 0},
    {S(-6, -5), S(-4, -2), S(-1, 0), S(1, 3), S(4, 4), S(9, 1), S(5, 0), S(-10, 0)},
    {S(-4, -4), S(-1, -2), 0, S(2, 2), S(3, 2), S(6, 1), S(2, 0), S(-8, 1)},
    {S(-3, -2), S(-4, -3), S(-4, -2), S(-4, 1), S(0, 1), S(3, 2), S(5, 3), S(7, 2)},
    {S(-2, -10), S(0, -9), S(0, -4), S(-1, 2), S(-1, 5), S(2, 5), S(-1, 7), S(3, 5)},
    {S(0, -5), S(0, -2), S(-1, 0), S(-3, 2), S(-1, 3), S(5, 3), S(4, 1), S(2, -4)},
};
const i32 pst_file[][8] = {
    {S(-2, 0), S(-1, 1), S(-1, 0), S(0, -1), S(1, 0), S(2, 0), S(2, 0), S(-1, -1)},
    {S(-5, -4), S(-2, -1), S(0, 2), S(2, 3), S(2, 3), S(2, 1), S(1, -1), S(-1, -4)},
    {S(-3, -2), 0, 0, S(0, 2), S(0, 2), S(0, 1), S(2, 0), S(0, -2)},
    {S(-2, 0), S(-2, 1), S(-1, 1), 0, S(1, 0), S(1, 0), S(3, -1), S(0, -1)},
    {S(-3, -5), S(-1, -2), S(-1, 0), S(-1, 1), S(0, 3), S(1, 3), S(3, 1), S(3, 0)},
    {S(-1, -3), S(2, -1), S(-3, 1), S(-5, 2), S(-5, 2), S(-3, 1), S(1, 0), S(2, -3)},
};
const i32 open_files[][5] = {
    {S(-1, 3), S(-6, 15), S(24, 19), S(6, 14), S(-26, 7)},
    {S(-4, -11), S(-10, -2), S(50, 13), S(-3, 31), S(-63, -6)},
};
const i32 pawn_protection[] = {S(24, 12), S(2, 17), S(2, 5), S(8, 4), S(-8, 11), S(-33, 24)};
const i32 passers[] = {S(-7, 14), S(17, 43), S(55, 98), S(207, 170)};
const i32 pawn_passed_protected = S(13, 18);
const i32 pawn_doubled = S(-12, -34);
const i32 pawn_phalanx = S(12, 12);
const i32 pawn_passed_blocked[] = {S(-7, -15), S(8, -37), S(9, -69), S(3, -67)};
const i32 pawn_passed_king_distance[] = {S(1, -5), S(-3, 9)};
const i32 bishop_pair = S(27, 67);
