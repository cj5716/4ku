...
const i32 max_material[] = {146, 516, 516, 945, 1780, 0, 0};
const i32 material[] = {S(84, 146), S(331, 516), S(346, 516), S(459, 945), S(982, 1780), 0};
const i32 pst_rank[] = {
    0,         S(-3, 0),  S(-3, -1), S(-1, -1), S(1, 0),  S(5, 3),  0,        0,          // Pawn
    S(-2, -5), S(0, -3),  S(1, -1),  S(3, 3),   S(4, 4),  S(5, 1),  S(2, 0),  S(-15, 2),  // Knight
    S(0, -2),  S(2, -1),  S(2, 0),   S(2, 0),   S(2, 0),  S(2, 0),  S(-1, 0), S(-10, 2),  // Bishop
    S(0, -3),  S(-1, -3), S(-2, -2), S(-2, 1),  S(0, 2),  S(2, 2),  S(1, 3),  S(2, 1),    // Rook
    S(2, -11), S(3, -8),  S(2, -3),  S(0, 2),   S(0, 5),  S(-1, 5), S(-4, 7), S(-3, 4),   // Queen
    S(0, -6),  S(1, -2),  S(-1, 0),  S(-4, 3),  S(-1, 5), S(5, 4),  S(4, 2),  S(5, -6),   // King
};
const i32 pst_file[] = {
    S(-1, 1),  S(-2, 1),  S(-1, 0), S(0, -1), S(1, 0),  S(2, 0),  S(2, 0),  S(-1, -1),  // Pawn
    S(-4, -3), S(-2, -1), S(0, 1),  S(2, 3),  S(2, 3),  S(2, 0),  S(1, 0),  S(-1, -3),  // Knight
    S(-2, -1), 0,         0,        S(0, 1),  S(1, 1),  S(0, 1),  S(1, 0),  S(-1, -1),  // Bishop
    S(-2, 0),  S(-1, 1),  S(0, 1),  S(1, 0),  S(1, -1), S(1, 0),  S(1, 0),  S(-1, 0),   // Rook
    S(-2, -3), S(-1, -1), S(-1, 1), S(0, 1),  S(0, 2),  S(1, 2),  S(2, 0),  S(1, -1),   // Queen
    S(-1, -5), S(2, -2),  S(-1, 1), S(-3, 2), S(-4, 2), S(-2, 1), S(2, -1), S(0, -5),   // King
};
...
const i32 king_attacks[] = {S(11, -5), S(19, -4), S(26, -9), S(17, 13), 0};
const i32 pawn_protection[] = {S(23, 16), S(2, 17), S(6, 19), S(8, 11), S(-7, 22), S(-30, 25)};
const i32 pawn_threat_penalty[] = {S(-4, 0), S(21, 1), S(11, 7), S(10, 20), S(9, 17), S(2, 10)};
const i32 passers[] = {S(13, 11), S(53, 45), S(100, 110), S(299, 189)};
const i32 pawn_passed_protected = S(11, 24);
const i32 pawn_doubled_penalty = S(10, 38);
const i32 pawn_phalanx = S(12, 16);
const i32 pawn_passed_blocked_penalty[] = {S(6, 19), S(-5, 44), S(-5, 83), S(72, 68)};
const i32 pawn_passed_king_distance[] = {S(-1, -6), S(-3, 11)};
const i32 bishop_pair = S(28, 84);
const i32 king_shield[] = {S(32, -10), S(24, -6)};
