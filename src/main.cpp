...
const i32 max_material[] = {138, 441, 444, 826, 1648, 0, 0};
const i32 material[] = {S(99, 138), S(331, 441), S(343, 444), S(458, 826), S(824, 1648), 0};
const i32 pst_rank[] = {
    0,         S(-2, 0),  S(-3, -1), S(-1, -1), S(1, 0),  S(4, 2), 0,        0,          // Pawn
    S(-4, -4), S(-2, -3), S(-1, -1), S(1, 3),   S(3, 3),  S(7, 1), S(5, 0),  S(-11, 1),  // Knight
    S(-2, -2), S(1, -2),  S(1, 0),   S(1, 0),   S(2, 1),  S(4, 0), S(1, 0),  S(-8, 2),   // Bishop
    S(-1, -4), S(-2, -4), S(-4, -2), S(-4, 1),  S(-1, 2), S(3, 2), S(3, 3),  S(6, 1),    // Rook
    S(1, -13), S(1, -10), S(0, -5),  S(-2, 1),  S(-1, 6), S(1, 5), S(-2, 9), S(1, 7),    // Queen
    S(0, -6),  S(0, -2),  S(-2, 0),  S(-4, 3),  S(-1, 4), S(5, 4), S(4, 2),  S(4, -4)    // King
};
const i32 pst_file[] = {
    S(-2, 1),  S(-1, 1),  S(-1, 0), S(0, -1), S(1, 0),  S(2, 0),  S(2, 0),  S(-1, -1),  // Pawn
    S(-5, -2), S(-2, -1), S(0, 1),  S(2, 3),  S(2, 2),  S(2, 0),  S(1, 0),  S(-1, -3),  // Knight
    S(-2, 0),  0,         S(1, 0),  S(0, 1),  S(1, 1),  S(-1, 1), S(1, 0),  S(0, -1),   // Bishop
    S(-2, 1),  S(-2, 1),  S(-1, 1), S(1, 0),  S(2, -1), S(1, 0),  S(2, 0),  S(-1, 0),   // Rook
    S(-3, -4), S(-2, -2), S(-1, 0), S(0, 1),  S(0, 2),  S(1, 3),  S(3, 1),  S(3, -1),   // Queen
    S(-3, -4), S(2, -2),  S(-2, 1), S(-3, 2), S(-4, 2), S(-1, 1), S(2, -1), S(0, -5)    // King
};
const i32 open_files[] =
    {S(2, 5), S(-3, 20), S(20, 16), S(4, 17), S(-23, 11), S(-3, -12), S(-11, 0), S(45, 1), S(-13, 31), S(-63, 1)};
const i32 mobilities[] = {S(8, 5), S(8, 7), S(4, 4), S(4, 3), S(-4, -1)};
const i32 pawn_protection[] = {S(24, 14), S(1, 16), S(9, 16), S(10, 8), S(-5, 23), S(-33, 25)};
const i32 passers[] = {S(6, 13), S(41, 47), S(78, 118), S(220, 203)};
const i32 pawn_passed_protected = S(11, 20);
const i32 pawn_doubled_penalty = S(14, 38);
const i32 pawn_phalanx = S(13, 11);
const i32 pawn_passed_blocked_penalty[] = {S(10, 14), S(-4, 41), S(-11, 84), S(5, 100)};
const i32 pawn_passed_king_distance[] = {S(0, -6), S(-3, 11)};
const i32 bishop_pair = S(30, 71);
const i32 king_shield[] = {S(35, -11), S(28, -7)};
