...
const i32 max_material[] = {157, 527, 508, 924, 1843, 0, 0};
const i32 material[] = {S(87, 157), S(301, 527), S(288, 508), S(381, 924), S(682, 1843), 0};
const i32 pst_rank[][8] = {
    {0, S(-2, 0), S(-2, -1), S(-1, -1), S(1, 0), S(4, 3), 0, 0},
    {S(-5, -6), S(-3, -3), S(-1, 0), S(1, 4), S(4, 5), S(9, 2), S(5, 0), S(-11, 0)},
    {S(-1, -2), S(1, -2), S(1, -1), S(1, 0), S(2, 1), S(4, 0), S(1, 0), S(-9, 2)},
    {S(-1, -4), S(-3, -4), S(-4, -3), S(-4, 1), S(0, 2), S(3, 2), S(3, 3), S(5, 2)},
    {S(1, -12), S(1, -11), S(0, -6), S(-1, 1), S(-1, 6), S(1, 6), S(-2, 9), S(1, 8)},
    {S(0, -6), S(0, -1), S(-2, 0), S(-3, 3), S(-1, 5), S(4, 4), S(4, 2), S(3, -4)},
};
const i32 pst_file[][8] = {
    {S(-1, 1), S(-2, 1), S(-1, 0), S(0, -1), S(1, 0), S(2, 0), S(2, 1), S(-1, -1)},
    {S(-5, -5), S(-2, -1), S(0, 2), S(2, 4), S(2, 4), S(2, 1), S(1, -1), S(-2, -5)},
    {S(-2, -1), S(0, -1), 0, S(0, 1), S(0, 1), S(0, 1), S(2, 0), S(0, -1)},
    {S(-2, 0), S(-2, 1), S(-1, 1), S(1, 0), S(1, 0), S(1, 0), S(3, -1), S(-1, -1)},
    {S(-2, -6), S(-2, -3), S(-1, -1), S(-1, 1), S(0, 3), S(1, 3), S(2, 2), S(3, 1)},
    {S(-1, -4), S(1, -1), S(-3, 1), S(-5, 3), S(-5, 2), S(-3, 2), S(2, 0), S(1, -4)},
};
const i32 open_files[][5] = {
    {S(2, 5), S(-2, 19), S(19, 17), S(5, 16), S(-27, 8)},
    {S(0, -12), S(-7, -2), S(43, 4), S(-7, 26), S(-64, -6)},
};
const i32 mobilities[] = {S(7, 8), S(3, 4), S(4, 3), 0};
const i32 pawn_protection[] = {S(23, 15), S(0, 18), S(6, 19), S(7, 8), S(-7, 23), S(-33, 29)};
const i32 passers[] = {S(-3, 19), S(22, 58), S(58, 131), S(200, 224)};
const i32 pawn_passed_protected = S(10, 21);
const i32 pawn_doubled = S(-11, -42);
const i32 pawn_phalanx = S(13, 14);
const i32 pawn_passed_blocked[] = {S(-9, -20), S(7, -48), S(9, -90), S(-3, -93)};
const i32 pawn_passed_king_distance[] = {S(1, -7), S(-3, 11)};
const i32 bishop_pair = S(25, 82);
const i32 king_shield[] = {S(35, -6), S(26, -7)};
