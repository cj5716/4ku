...
const i32 max_material[] = {139, 447, 449, 832, 1667, 0, 0};
const i32 material[] = {S(96, 139), S(317, 447), S(328, 449), S(438, 832), S(776, 1667), 0};
const i32 pst_rank[] = {
    0,         S(-2, 0),  S(-3, -1), S(-1, -1), S(1, 0),  S(4, 2), 0,        0,          // Pawn
    S(-4, -4), S(-2, -3), S(-1, -1), S(1, 3),   S(3, 4),  S(7, 1), S(5, 0),  S(-10, 1),  // Knight
    S(-2, -2), S(1, -2),  S(1, 0),   S(1, 0),   S(2, 1),  S(4, 0), S(1, 0),  S(-8, 2),   // Bishop
    S(-1, -4), S(-2, -4), S(-3, -2), S(-4, 1),  S(-1, 2), S(3, 2), S(3, 3),  S(6, 1),    // Rook
    S(1, -13), S(1, -10), S(0, -5),  S(-1, 1),  S(-1, 6), S(1, 5), S(-2, 9), S(1, 7),    // Queen
    S(0, -6),  S(0, -2),  S(-2, 0),  S(-4, 3),  S(0, 4),  S(5, 4), S(3, 3),  S(4, -4)    // King
};
const i32 pst_file[] = {
    S(-2, 1),  S(-1, 1),  S(-1, 0), S(0, -1), S(1, 0),  S(2, 0),  S(2, 0),  S(-1, -1),  // Pawn
    S(-4, -3), S(-2, 0),  S(0, 1),  S(2, 3),  S(2, 2),  S(2, 0),  S(1, 0),  S(-1, -3),  // Knight
    S(-2, 0),  0,         S(1, 0),  S(0, 1),  S(1, 1),  S(-1, 1), S(1, 0),  S(0, -1),   // Bishop
    S(-2, 0),  S(-2, 1),  S(-1, 1), S(1, 0),  S(2, -1), S(1, 0),  S(2, 0),  S(-1, 0),   // Rook
    S(-3, -4), S(-2, -2), S(-1, 0), S(-1, 1), S(0, 2),  S(1, 3),  S(3, 1),  S(3, 0),    // Queen
    S(-2, -4), S(2, -1),  S(-2, 1), S(-3, 2), S(-4, 2), S(-2, 1), S(2, -1), S(0, -5)    // King
};
const i32 open_files[] =
    {S(2, 5), S(-3, 19), S(19, 16), S(4, 16), S(-23, 11), S(-3, -12), S(-10, 0), S(44, 1), S(-11, 29), S(-62, 1)};
const i32 mobilities[] = {S(8, 5), S(8, 7), S(4, 4), S(4, 3), S(-4, -1)};
const i32 pawn_protection[] = {S(23, 14), S(0, 16), S(9, 16), S(9, 8), S(-6, 24), S(-33, 24)};
const i32 passers[] = {S(7, 14), S(42, 48), S(80, 117), S(224, 201)};
const i32 pawn_passed_protected = S(11, 20);
const i32 pawn_doubled_penalty = S(14, 38);
const i32 pawn_phalanx = S(13, 12);
const i32 pawn_passed_blocked_penalty[] = {S(11, 14), S(-4, 41), S(-13, 84), S(6, 96)};
const i32 pawn_passed_king_distance[] = {S(0, -6), S(-3, 11)};
const i32 bishop_pair = S(29, 71);
