...
const i32 max_material[] = {146, 516, 516, 944, 1759, 0, 0};
const i32 material[] = {S(85, 146), S(337, 516), S(349, 516), S(463, 944), S(1015, 1759), 0};
const i32 pst_rank[] = {
    0,         S(-2, 0),  S(-3, -1), S(0, -1), S(1, 0),  S(4, 3),  0,        0,          // Pawn
    S(-2, -5), S(0, -3),  S(2, -1),  S(3, 3),  S(4, 4),  S(5, 2),  S(2, 0),  S(-14, 1),  // Knight
    S(0, -2),  S(2, -1),  S(2, 0),   S(3, 0),  S(3, 0),  S(2, 0),  S(-1, 0), S(-12, 3),  // Bishop
    S(0, -3),  S(-1, -3), S(-2, -2), S(-2, 1), S(0, 2),  S(2, 2),  S(0, 3),  S(3, 1),    // Rook
    S(3, -11), S(3, -8),  S(2, -3),  S(0, 2),  S(-1, 5), S(-1, 5), S(-4, 7), S(-2, 4),   // Queen
    S(-3, -5), S(-2, -2), S(-3, 0),  S(-5, 3), S(-2, 4), S(3, 4),  S(5, 1),  S(5, -6),   // King
};
const i32 pst_file[] = {
    S(-1, 1),  S(-2, 1),  S(-1, 0), S(0, -1), S(1, 0),  S(2, 0),  S(2, 0),  S(-1, -1),  // Pawn
    S(-4, -3), S(-1, -1), S(0, 2),  S(2, 3),  S(2, 3),  S(2, 0),  S(1, 0),  S(-1, -3),  // Knight
    S(-2, -1), 0,         S(1, 0),  S(0, 1),  S(1, 1),  S(0, 1),  S(1, 0),  S(0, -1),   // Bishop
    S(-2, 0),  S(-1, 1),  S(0, 1),  S(1, 0),  S(2, -1), S(1, 0),  S(1, 0),  S(-2, 0),   // Rook
    S(-2, -3), S(-1, -1), S(-1, 1), S(0, 1),  S(0, 2),  S(1, 2),  S(2, -1), S(1, -1),   // Queen
    S(-3, -4), S(2, -1),  S(-2, 1), S(-4, 2), S(-4, 3), S(-2, 2), S(2, 0),  S(0, -5),   // King
};
...
const i32 king_attacks[] = {S(12, -6), S(18, -3), S(25, -8), S(18, 12), 0};
const i32 pawn_protection[] = {S(22, 16), S(1, 18), S(6, 17), S(8, 9), S(-8, 25), S(-29, 24)};
const i32 pawn_threat_penalty[] = {S(-3, -1), S(22, 0), S(12, 5), S(11, 21), S(8, 17), S(2, 9)};
const i32 passers[] = {S(17, 10), S(62, 42), S(117, 106), S(297, 193)};
const i32 pawn_passed_protected = S(12, 23);
const i32 pawn_doubled_penalty = S(11, 38);
const i32 pawn_phalanx = S(12, 16);
const i32 pawn_passed_blocked_penalty[] = {S(9, 19), S(-5, 43), S(-10, 84), S(46, 87)};
const i32 pawn_passed_king_distance[] = {S(-1, -6), S(-3, 11)};
const i32 bishop_pair = S(27, 83);
