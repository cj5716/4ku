...
const i32 max_material[] = {142, 436, 471, 839, 1596, 0, 0};
const i32 material[] = {S(66, 142), S(256, 436), S(268, 471), S(314, 839), S(692, 1596), 0};
const i32 pst_rank[][8] = {
    {0, S(-2, 0), S(-3, -1), S(-1, -1), S(2, 0), S(5, 2), 0, 0},
    {S(-3, -6), S(-1, -2), S(1, 0), S(3, 3), S(5, 4), S(7, 2), S(3, 0), S(-16, -1)},
    {S(-1, -4), S(2, -2), S(3, 0), S(4, 1), S(3, 2), S(3, 1), 0, S(-15, 1)},
    {S(2, -4), S(0, -5), S(-1, -3), S(-2, 0), S(1, 1), S(3, 2), S(4, 3), S(-5, 5)},
    {S(0, -15), S(2, -10), S(1, -5), S(0, 2), S(-1, 6), S(1, 5), S(-2, 8), S(-1, 8)},
    {S(0, -6), S(0, -1), S(-2, 1), S(-5, 3), S(-3, 5), S(3, 4), S(4, 3), S(7, -4)},
};
const i32 pst_file[][8] = {
    {S(-1, 0), S(0, 1), 0, S(0, -1), S(1, 0), S(1, 0), S(2, 0), S(-2, 0)},
    {S(-4, -4), S(-1, -1), S(1, 2), S(1, 4), S(2, 3), S(2, 1), S(1, -1), S(-2, -5)},
    {S(-2, -2), 0, S(1, 0), S(0, 2), S(0, 2), S(-1, 1), S(2, -1), S(0, -3)},
    {S(-1, 0), S(-2, 1), S(-1, 1), S(0, 1), S(1, -1), S(1, 0), S(2, -1), S(1, -2)},
    {S(-3, -4), S(-1, -2), S(-1, 1), S(0, 2), S(0, 3), S(0, 3), S(2, 0), S(2, -2)},
    {S(-1, -4), S(2, -1), S(-2, 1), S(-5, 3), S(-4, 2), S(-2, 2), S(3, 0), S(1, -3)},
};
const i32 open_files[][5] = {
    {S(-1, 8), S(-7, 18), S(26, 25), S(6, 20), S(-22, 6)},
    {S(-4, -11), S(-16, -1), S(55, 8), S(-13, 45), S(-65, -6)},
};
const i32 pawn_protection[] = {S(24, 17), S(10, 20), S(7, 14), S(15, 8), S(-7, 19), S(-45, 35)};
const i32 passers[] = {S(-9, 16), S(7, 50), S(24, 121), S(122, 233)};
const i32 pawn_passed_protected = S(18, 16);
const i32 pawn_doubled = S(-11, -37);
const i32 pawn_phalanx = S(12, 12);
const i32 pawn_passed_blocked[] = {S(-9, -17), S(6, -39), S(14, -76), S(5, -118)};
const i32 pawn_passed_king_distance[] = {S(1, -7), S(-3, 12)};
const i32 bishop_pair = S(32, 59);
const i32 king_shield[] = {S(41, -9), S(31, -11)};
