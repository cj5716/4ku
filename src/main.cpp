...
const i32 max_material[] = {136, 432, 433, 818, 1625, 0, 0};
const i32 material[] = {S(94, 136), S(344, 432), S(353, 433), S(469, 818), S(844, 1625), 0};
const i32 pst_rank[] = {
    0,         S(-3, 0),  S(-3, -1), S(-1, -1), S(1, 0),  S(5, 2), 0,        0,          // Pawn
    S(-3, -4), S(-1, -3), S(1, -1),  S(2, 2),   S(3, 3),  S(6, 1), S(4, 0),  S(-12, 1),  // Knight
    S(-1, -2), S(2, -1),  S(2, 0),   S(2, 0),   S(2, 1),  S(3, 0), 0,        S(-9, 2),   // Bishop
    S(0, -3),  S(-1, -3), S(-2, -2), S(-3, 0),  S(0, 2),  S(2, 2), S(1, 3),  S(3, 1),    // Rook
    S(2, -11), S(3, -9),  S(1, -4),  S(-1, 1),  S(-1, 5), S(0, 5), S(-3, 7), S(-1, 6),   // Queen
    S(-1, -5), S(1, -2),  0,         S(-2, 2),  S(1, 4),  S(6, 4), S(3, 3),  S(2, -3)    // King
};
const i32 pst_file[] = {
    S(-1, 1),  S(-2, 1),  S(-1, 0), S(0, -1), S(1, 0),  S(2, 0),  S(2, 0),  S(-1, -1),  // Pawn
    S(-5, -3), S(-1, -1), S(0, 1),  S(2, 3),  S(2, 2),  S(2, 0),  S(1, 0),  S(-1, -3),  // Knight
    S(-2, 0),  0,         S(1, 0),  S(0, 1),  S(1, 1),  S(0, 1),  S(2, 0),  S(0, -1),   // Bishop
    S(-2, 0),  S(-1, 0),  S(0, 1),  S(1, 0),  S(2, -1), S(1, 0),  S(1, 0),  S(-2, 0),   // Rook
    S(-2, -4), S(-1, -2), S(-1, 0), S(0, 1),  S(0, 2),  S(1, 2),  S(2, 0),  S(2, -1),   // Queen
    S(-3, -5), S(2, -2),  S(-2, 1), S(-2, 1), S(-3, 2), S(-1, 1), S(2, -1), S(0, -5)    // King
};
...
const i32 mobilities[] = {S(8, 5), S(7, 7), S(3, 4), S(4, 2), S(-4, 0)};
const i32 king_attacks[] = {S(11, -6), S(19, -5), S(27, -10), S(21, 2), 0};
const i32 pawn_protection[] = {S(22, 13), S(0, 15), S(7, 16), S(8, 10), S(-6, 21), S(-31, 24)};
const i32 pawn_threat_penalty[] = {S(-4, 1), S(22, 1), S(12, 3), S(12, 17), S(9, 15), S(6, 5)};
const i32 passers[] = {S(8, 12), S(42, 45), S(77, 115), S(209, 243)};
const i32 pawn_passed_protected = S(9, 20);
const i32 pawn_doubled_penalty = S(10, 37);
const i32 pawn_phalanx = S(12, 11);
const i32 pawn_passed_blocked_penalty[] = {S(8, 13), S(-4, 39), S(-11, 81), S(-10, 158)};
