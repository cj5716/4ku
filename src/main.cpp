...
const i32 max_material[] = {147, 494, 523, 903, 1784, 0, 0};
const i32 material[] = {S(90, 147), S(308, 494), S(323, 523), S(393, 903), S(705, 1784), 0};
const i32 pst_rank[][8] = {
    {0, S(-3, 0), S(-3, -1), S(-1, -1), S(2, 0), S(5, 2), 0, 0},
    {S(-6, -6), S(-4, -2), S(-1, 0), S(1, 4), S(4, 4), S(9, 1), S(5, 0), S(-9, -1)},
    {S(-4, -4), S(-1, -2), 0, S(2, 2), S(3, 3), S(6, 1), S(2, 0), S(-8, 1)},
    {S(-2, -3), S(-3, -4), S(-4, -3), S(-4, 0), S(-1, 2), S(3, 2), S(4, 3), S(7, 2)},
    {S(-2, -12), S(0, -10), S(0, -5), S(-1, 2), S(-1, 6), S(2, 5), S(-1, 8), S(3, 6)},
    {S(0, -6), S(0, -2), S(-2, 0), S(-5, 3), S(-3, 5), S(2, 5), S(1, 3), S(1, -3)},
};
const i32 pst_file[][8] = {
    {S(-2, 0), S(-1, 1), S(-1, 0), S(0, -1), S(1, 0), S(2, 0), S(2, 0), S(-1, -1)},
    {S(-5, -4), S(-2, -1), S(0, 2), S(2, 4), S(2, 3), S(2, 1), S(1, -1), S(-1, -5)},
    {S(-3, -2), 0, 0, S(0, 2), S(0, 2), S(0, 1), S(2, 0), S(0, -3)},
    {S(-2, 0), S(-2, 1), S(-1, 1), 0, S(1, -1), S(1, 0), S(3, -1), S(-1, -1)},
    {S(-3, -5), S(-1, -2), S(-1, 0), S(0, 2), S(0, 3), S(1, 3), S(2, 1), S(2, -1)},
    {S(-1, -4), S(2, -1), S(-3, 1), S(-5, 2), S(-5, 2), S(-2, 1), S(2, 0), S(1, -4)},
};
const i32 open_files[][5] = {
    {S(0, 3), S(-7, 18), S(24, 22), S(6, 17), S(-27, 9)},
    {S(-5, -13), S(-11, -2), S(52, 15), S(-6, 41), S(-65, -5)},
};
const i32 pawn_protection[] = {S(24, 14), S(3, 18), S(2, 5), S(9, 3), S(-9, 14), S(-31, 27)};
const i32 passers[] = {S(-5, 17), S(18, 53), S(52, 121), S(202, 203)};
const i32 pawn_passed_protected = S(12, 20);
const i32 pawn_doubled = S(-11, -40);
const i32 pawn_phalanx = S(12, 13);
const i32 pawn_passed_blocked[] = {S(-8, -17), S(8, -44), S(12, -83), S(8, -85)};
const i32 pawn_passed_king_distance[] = {S(1, -7), S(-3, 11)};
const i32 bishop_pair = S(28, 74);
const i32 king_shield[] = {S(38, -8), S(29, -8)};
