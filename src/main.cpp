...
const i32 max_material[] = {147, 466, 468, 869, 1710, 0, 0};
const i32 material[] = {S(101, 147), S(346, 466), S(359, 468), S(479, 869), S(907, 1710), 0};
const i32 pst_rank[][8] = {
    {0, S(-2, 0), S(-3, -1), S(-1, -1), S(1, 0), S(4, 3), 0, 0},
    {S(-4, -5), S(-2, -3), S(-1, -1), S(2, 3), S(4, 4), S(8, 1), S(5, 0), S(-12, 1)},
    {S(-2, -2), S(1, -2), S(1, 0), S(1, 0), S(2, 1), S(4, 0), S(1, 0), S(-8, 2)},
    {S(-2, -4), S(-2, -4), S(-4, -2), S(-4, 1), S(-1, 2), S(3, 2), S(3, 3), S(6, 1)},
    {S(1, -13), S(1, -10), S(0, -5), S(-1, 2), S(-1, 6), S(1, 6), S(-2, 9), S(1, 7)},
    {S(0, -6), S(0, -2), S(-1, 0), S(-3, 3), S(0, 4), S(6, 4), S(4, 2), S(5, -4)},
};
const i32 pst_file[][8] = {
    {S(-2, 1), S(-1, 1), S(-1, 0), S(0, -1), S(1, 0), S(2, 0), S(2, 0), S(-1, -1)},
    {S(-4, -3), S(-2, -1), S(0, 1), S(2, 3), S(2, 3), S(2, 0), S(1, -1), S(-1, -3)},
    {S(-2, 0), S(0, -1), S(1, 0), S(0, 1), S(0, 1), S(-1, 1), S(2, -1), S(0, -1)},
    {S(-2, 0), S(-2, 1), S(-1, 1), S(1, 0), S(1, -1), S(1, 0), S(2, -1), S(-1, -1)},
    {S(-3, -5), S(-2, -3), S(-1, 0), S(0, 1), S(0, 2), S(1, 3), S(2, 2), S(3, 0)},
    {S(-1, -5), S(2, -2), S(-2, 1), S(-4, 2), S(-4, 2), S(-2, 1), S(2, -1), S(1, -5)},
};
const i32 open_files[][5] = {
    {S(2, 5), S(-4, 21), S(20, 16), S(3, 19), S(-24, 10)},
    {S(-2, -14), S(-9, -1), S(48, 0), S(-12, 33), S(-63, 0)},
};
const i32 mobilities[] = {S(9, 5), S(8, 7), S(4, 5), S(4, 3), S(-4, -1)};
const i32 pawn_protection[] = {S(24, 15), S(1, 18), S(8, 18), S(9, 9), S(-6, 24), S(-37, 27)};
const i32 passers[] = {S(-2, 18), S(26, 56), S(59, 133), S(200, 224)};
const i32 pawn_passed_protected = S(12, 20);
const i32 pawn_doubled = S(-15, -40);
const i32 pawn_phalanx = S(14, 13);
const i32 pawn_passed_blocked[] = {S(-11, -15), S(2, -44), S(6, -87), S(6, -106)};
const i32 pawn_passed_king_distance[] = {S(1, -7), S(-4, 12)};
const i32 bishop_pair = S(31, 75);
const i32 king_shield[] = {S(36, -12), S(28, -8)};
