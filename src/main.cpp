...
i32 alphabeta(Position &pos,
    ...
    if (ply > 0 && !in_qsearch && !in_check && alpha == beta - 1) {
        ...
        if (depth < 7) {
            ...
            in_qsearch = static_eval + 245 * depth < alpha;
        }
        ...
        if (depth > 1 && static_eval >= beta && do_null && pos.colour[0] & ~(pos.pieces[Pawn] | pos.pieces[King])) {
            ...
            if (-alphabeta(npos,
                           -beta,
                           -alpha,
                           depth - 4 - depth / 6 - min((static_eval - beta) / 201, 3),
                           ply + 1,
                           // minify enable filter delete
                           nodes,
                           // minify disable filter delete
    ...
    for (i32 i = 0; i < num_moves; ++i) {
        ...
        // Delta pruning
        if (in_qsearch && !in_check && static_eval + 49 + gain < alpha)
            break;

        // Forward futility pruning
        if (ply > 0 && depth < 7 && !in_qsearch && !in_check && num_moves_evaluated &&
            static_eval + 93 * depth + gain < alpha)
            break;
        ...
        full_window:
            ...
            i32 reduction = depth > 2 && num_moves_evaluated > 4 && !gain
                                ? num_moves_evaluated / 13 + depth / 16 + (alpha == beta - 1) + !improving +
...
auto iteratively_deepen(Position &pos,
    ...
    research:
        ...
        if (!research && now() >= start_time + allocated_time / 9)
            break;
    }
    return stack[0].move;
}
