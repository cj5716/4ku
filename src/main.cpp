...
const i32 max_material[] = {139, 452, 454, 848, 1729, 0, 0};
const i32 material[] = {S(95, 139), S(332, 452), S(344, 454), S(455, 848), S(768, 1729), 0};
const i32 pst_rank[] = {
    0,         S(-3, 0),  S(-3, -1), S(-1, -1), S(1, 0),  S(5, 2), 0,        0,          // Pawn
    S(-2, -4), S(0, -3),  S(1, -1),  S(3, 2),   S(3, 3),  S(6, 1), S(3, 0),  S(-13, 1),  // Knight
    S(-1, -1), S(2, -1),  S(2, 0),   S(2, 0),   S(2, 0),  S(3, 0), 0,        S(-8, 2),   // Bishop
    S(0, -3),  S(-1, -3), S(-2, -2), S(-2, 0),  S(0, 2),  S(2, 2), S(1, 3),  S(1, 1),    // Rook
    S(3, -11), S(3, -8),  S(2, -4),  S(0, 1),   S(-1, 5), S(0, 5), S(-3, 6), S(-5, 6),   // Queen
    S(-1, -5), S(1, -2),  0,         S(-2, 2),  S(1, 4),  S(6, 4), S(3, 3),  S(2, -4),   // King
};
const i32 pst_file[] = {
    S(-1, 1),  S(-2, 1),  S(-1, 0), S(0, -1), S(1, 0),  S(2, 0),  S(2, 0),  S(-1, 0),   // Pawn
    S(-4, -2), S(-2, -1), S(0, 1),  S(2, 3),  S(2, 2),  S(2, 0),  S(1, 0),  S(-1, -3),  // Knight
    S(-2, 0),  0,         S(1, 0),  S(0, 1),  S(1, 1),  S(-1, 1), S(2, 0),  S(0, -1),   // Bishop
    S(-2, 0),  S(-1, 1),  S(0, 1),  S(1, 0),  S(2, -1), S(1, 0),  S(1, 0),  S(-2, 0),   // Rook
    S(-2, -3), S(-1, -1), S(-1, 0), S(0, 1),  S(0, 2),  S(1, 2),  S(2, 1),  S(1, -2),   // Queen
    S(-2, -5), S(3, -2),  S(-1, 1), S(-3, 2), S(-3, 2), S(-1, 1), S(2, -1), S(0, -5),   // King
};
...
const i32 mobilities[] = {S(9, 6), S(8, 7), S(3, 5), S(4, 2), S(-5, -1)};
const i32 king_attacks[] = {S(10, -4), S(19, -5), S(26, -9), S(18, 9), 0};
const i32 pawn_protection[] = {S(22, 15), S(2, 14), S(6, 18), S(8, 9), S(-5, 19), S(-30, 24)};
const i32 pawn_threat_penalty[] = {S(-4, 0), S(21, 1), S(12, 5), S(10, 17), S(9, 17), S(4, 6)};
const i32 passers[] = {S(6, 12), S(38, 47), S(73, 119), S(215, 250)};
const i32 pawn_passed_protected = S(11, 18);
const i32 pawn_doubled_penalty = S(11, 37);
const i32 pawn_phalanx = S(12, 12);
const i32 pawn_passed_blocked_penalty[] = {S(9, 15), S(-6, 42), S(-6, 83), S(-5, 159)};
const i32 pawn_passed_king_distance[] = {S(0, -6), S(-3, 11)};
const i32 bishop_pair = S(32, 73);
