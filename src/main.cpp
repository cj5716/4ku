...
const i32 pawn_attacked_penalty[] = {S(60, 14), S(155, 140)};
...
i32 alphabeta(Position &pos,
    ...
    if (ply > 0 && !in_qsearch && !in_check && alpha == beta - 1) {
        ...
        if (depth < 9) {
            assert(ply > 0);
            if (static_eval - 70 * (depth - improving) >= beta)
                return static_eval;

            in_qsearch = static_eval + 232 * depth < alpha;
        }
        ...
        if (depth > 2 && static_eval >= beta && static_eval >= stack[ply].score && do_null &&
            ...
            if (-alphabeta(npos,
                           -beta,
                           -alpha,
                           depth - 4 - depth / 4 - min((static_eval - beta) / 195, 3),
                           ply + 1,
                           // minify enable filter delete
                           nodes,
                           // minify disable filter delete
    ...
    for (i32 i = 0; i < num_moves; ++i) {
        ...
        full_window:
            ...
            i32 reduction = depth > 2 && num_moves_evaluated > 4 && !gain
                                ? num_moves_evaluated / 12 + depth / 15 + (alpha == beta - 1) + !improving +
