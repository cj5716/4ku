...
const i32 max_material[] = {151, 486, 518, 916, 1730, 0, 0};
const i32 material[] = {S(113, 151), S(445, 486), S(468, 518), S(583, 916), S(1203, 1730), 0};
const i32 pst_rank[][8] = {
    {0, S(-3, 0), S(-3, -1), S(-1, -1), S(2, 0), S(5, 2), 0, 0},
    {S(-5, -6), S(-3, -2), 0, S(2, 4), S(5, 4), S(11, 1), S(6, -1), S(-17, 0)},
    {S(-5, -4), S(-1, -2), S(1, 0), S(2, 2), S(2, 3), S(7, 1), S(2, 0), S(-9, 0)},
    {S(-2, -3), S(-4, -4), S(-4, -3), S(-4, 0), S(-1, 2), S(3, 2), S(4, 3), S(8, 2)},
    {S(-1, -13), S(0, -10), S(0, -5), S(-1, 2), S(-1, 7), S(3, 4), S(-1, 8), S(2, 6)},
    {S(0, -5), S(0, -1), S(-2, 1), S(-4, 3), S(-1, 4), S(6, 3), S(5, 2), S(6, -4)},
};
const i32 pst_file[][8] = {
    {S(-2, 1), S(-1, 1), S(-1, 0), S(0, -1), S(1, 0), S(2, 0), S(3, 0), S(-2, -1)},
    {S(-6, -4), S(-1, -1), S(1, 2), S(2, 4), S(2, 4), S(3, 1), S(1, -1), S(-2, -5)},
    {S(-3, -2), 0, S(1, 0), S(0, 2), S(0, 2), S(0, 1), S(3, -1), S(-1, -3)},
    {S(-2, 0), S(-2, 1), S(-1, 1), 0, S(1, -1), S(1, 0), S(2, -1), S(0, -2)},
    {S(-3, -5), S(-2, -3), S(-1, 0), S(0, 1), S(0, 3), S(1, 4), S(3, 1), S(3, -2)},
    {S(0, -4), S(3, -1), S(-2, 2), S(-4, 3), S(-5, 3), S(-1, 2), S(2, 0), S(2, -3)},
};
const i32 open_files[][5] = {
    {S(-1, 5), S(-7, 19), S(28, 23), S(5, 23), S(-30, 10)},
    {S(-3, -15), S(-12, -3), S(60, 14), S(-11, 52), S(-76, -3)},
};
const i32 pawn_protection[] = {S(27, 15), S(6, 17), S(2, 6), S(10, 4), S(-8, 13), S(-37, 32)};
const i32 passers[] = {S(-8, 17), S(14, 58), S(37, 142), S(180, 241)};
const i32 pawn_passed_protected = S(17, 20);
const i32 pawn_doubled = S(-16, -41);
const i32 pawn_phalanx = S(14, 13);
const i32 pawn_passed_blocked[] = {S(-12, -15), S(2, -42), S(10, -89), S(35, -118)};
const i32 pawn_passed_king_distance[] = {S(2, -8), S(-4, 13)};
const i32 bishop_pair = S(37, 77);
const i32 king_shield[] = {S(45, -14), S(34, -11)};
